...
void CTrack::CalCubicBSplines()
{
	...
	float distance = 0;
	int last = 0;
	std::vector<Pnt3f> tmpPts;
	std::vector<Pnt3f> tmpVecs;
	tmpPts.push_back(splinePts[0]);
	tmpVecs.push_back(splineVecs[0]);
	for (size_t i = 1; (last + i) < this->splinePts.size(); ++i) {
		float xDis = (splinePts[(last + i) % splinePts.size()].x - splinePts[(last + i + 1) % splinePts.size()].x);
		float yDis = (splinePts[(last + i) % splinePts.size()].y - splinePts[(last + i + 1) % splinePts.size()].y);
		float zDis = (splinePts[(last + i) % splinePts.size()].z - splinePts[(last + i + 1) % splinePts.size()].z);
		distance += sqrt(pow(xDis, 2) + pow(yDis, 2) + pow(zDis, 2));
		if (distance >= 0.5) {
			last = (last + i) % splinePts.size();
			i = 1;
			tmpPts.push_back(splinePts[last]);
			tmpVecs.push_back(splineVecs[last]);
			distance = 0;
		}
	}
	splinePts = tmpPts;
...
void CTrack::drawTrain(bool doingShadows)
{
	...
	trainPos.push_back(splinePts[index]);
	...
	float theta4 = radiansToDegrees(atan2(Dot(W0, splineVecs[index%splineVecs.size()]) / W0.length(), Dot(U0, splineVecs[index%splineVecs.size()]) / U0.length()));
	float theta2 = radiansToDegrees(asin(cross.y));
	glRotatef(theta2, 0, 0, 1);
	trainRotate.push_back(Pnt3f(theta3, theta1, theta2));
	...
	for (int n = 1, last = index; n < trainNums; n++) {
		...
		for (int i = 0;; i++) {
			...
			if (distance > size * 4.5) {
				...
				trainPos.push_back(splinePts[last]);
				float theta1 = -radiansToDegrees(atan2(cross.z, cross.x));
				glRotatef(theta1, 0, 1, 0);
				...
				float theta4 = radiansToDegrees(atan2(Dot(W0, splineVecs[last%splineVecs.size()]) / W0.length(), Dot(U0, splineVecs[last%splineVecs.size()]) / U0.length()));
				float theta2 = radiansToDegrees(asin(cross.y));
				glRotatef(theta2, 0, 0, 1);
				trainRotate.push_back(Pnt3f(theta3, theta1, theta2));
				...
				if (n == trainNums - 1 && trainSpeed != 0) {
					/*trainCam = splinePts[last];
					trainCam.z += size * 2;
					trainCam.y += trainH;*/
					float slope = getSlope(xD, yD, zD);

					if (slope > 0) {
						if (trainSpeed + physicOffset >= 0.000001 && trainSpeed + physicOffset <= 0.005) {
							physicOffset += 0.0000005;
						}
						else
							physicOffset -= 0.0000005;
					}
					else if (slope < 0) {
						if (trainSpeed + physicOffset >= 0.0000011 && trainSpeed + physicOffset <= 0.005) {
							physicOffset -= 0.000001;
						}
						else
							physicOffset += 0.0001;
					}
						...
						if (physicOffset < 0)
							physicOffset += 0.000005;
						else if (physicOffset > 0)
							physicOffset -= 0.000005;
					}
	...
	trainU += (trainSpeed + physicOffset);
	if (trainU < 0) trainU += 1;
	if (trainU > 1) trainU -= 1;
}
		...
		for (int i = 0;; i++) {
			...
			if (distance > size * 4.5) {
				...
				if (n == trainNums - 1) {
					...
					trainLookAt.camPosition = splinePts[last];
					trainLookAt.camPosition = trainLookAt.camPosition + trainH / splineVecs[last%splineVecs.size()].length() * splineVecs[last%splineVecs.size()];
					trainLookAt.camDirection = vecY;
					trainLookAt.camUp = splineVecs[last%splineVecs.size()];
...
void CTrack::drawTunnel()
{
# define M_PI 3.14159265358979323846/* pi */
	float r = 12;
	const int steps = 32;
	int end = (splinePts.size() - 1) * 0.3;
	Pnt3f last[steps];

	glPushMatrix();
	glBegin(GL_TRIANGLE_STRIP);
	for (int i = 0; i < end; i++) {
		Pnt3f mid = Pnt3f((splinePts[i].x + splinePts[i+1].x)/2, (splinePts[i].y + splinePts[i + 1].y) / 2, (splinePts[i].z + splinePts[i + 1].z) / 2);

		Pnt3f vec = Pnt3f(splinePts[i + 1].x - splinePts[i].x, splinePts[i + 1].y - splinePts[i].y, splinePts[i + 1].z - splinePts[i].z);
		Pnt3f cross, up;
		cross.x = vec.y * splineVecs[i].z - vec.z * splineVecs[i].y;
		cross.y = vec.z * splineVecs[i].x - vec.x * splineVecs[i].z;
		cross.z = vec.x * splineVecs[i].y - vec.y * splineVecs[i].x;
		...
		Pnt3f vec1 = Pnt3f(splinePts[i + 2].x - splinePts[i + 1].x, splinePts[i + 2].y - splinePts[i + 1].y, splinePts[i + 2].z - splinePts[i + 1].z);
		Pnt3f cross1, up1;
		cross1.x = vec1.y * splineVecs[i+1].z - vec1.z * splineVecs[i+1].y;
		cross1.y = vec1.z * splineVecs[i+1].x - vec1.x * splineVecs[i+1].z;
		cross1.z = vec1.x * splineVecs[i+1].y - vec1.y * splineVecs[i+1].x;
		cross1.normalize();
		up1 = splineVecs[i + 1];
		up1.normalize();
		...
		for (int j = 0; j <= steps; j++) {
			...
			pt0 = splinePts[i] + (cross * cos(angle) + up * sin(angle)) * r;
			pt1 = splinePts[i+1] + (cross1 * cos(angle) + up1 * sin(angle)) * r;
...
float CTrack::getSlope(float x, float y, float z)
{
	return y / std::sqrt(x * x + z * z);
}
